...
%	For each generated sequence, the list is retrieved and appended into a single list.
...
sequenceReport([Head|Tail]) :- 
	...
	permutationRule([Head|Tail], Permutation),
	%	Initially print Permutation to demonstrate which part we're counting
	write('Count for '), print(Permutation), write(':'), nl,
