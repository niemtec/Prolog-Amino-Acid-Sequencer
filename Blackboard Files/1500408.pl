...
aminoLong(X) :-
	aminoname(_,Y,X),
	write(X), write(' is known as '), write(Y).
...
% Returns all values (as set X) which satisfy the condition of 'findall'
% function through unification. If aminoclass is small and neutral, take
% aminoclass short name, store as Y. Compare Y with aminoname and
% present matching long name as Z. Value of Z is then parsed as X and
% printed.
smallNeutralLong(X) :-
	setof(Z,(aminoclass(Y,_,'small'), aminoclass(Y,_,'neutral'), aminoname(_,Z,Y)), X).
